...
from utils.finn_schemas import (
    NEWBUILDINGS
)
...
def transform_finn_ads_metadata_file(client: Minio, search_key: str, ingest_object: Object, storage_options):
    ...
    # if search_key == "NEWBUILDINGS":
    if search_key == "SEARCH_ID_REALESTATE_NEWBUILDINGS":
        df = NEWBUILDINGS(df)
    else:
        raise Exception(f"No schema definition for search_id: {search_key}")
